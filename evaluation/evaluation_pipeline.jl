...
# europe_data = readtable("../data/EuropeJobs.csv"); X = europe_data[2:end];  y = ones(size(X,1)); truelabels = false;
...
if gridsearch
	...
	lnr = OptimalTrees.OptimalTreeClassifier(ls_num_tree_restarts = 100, ls_random_seed = s, cp = 0.0, max_depth = 3,
		minbucket = 5, criterion = :silhouette, show_progress_bar=true);
end
...
optclust_score = OptimalTrees.score(lnr, X, y, criterion = :silhouette);
