...
function run_single(;data::String=data,
	...
	results = DataFrame(seed = s, K_optclust=leaf_cnt, kmeans_k=kmeans_best, true_k=true_k ,
	...
	filepath_lnr = joinpath(resultsfolderpath, "lnr$data-$criterion-$method-lnr.jld")
    @save filepath_lnr lnr
    filepath_accuracy = joinpath(resultsfolderpath, "results$data-$criterion-$method.csv")
