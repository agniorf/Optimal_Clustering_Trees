# seed = 2; gridsearch = false; num_tree_restarts = 100; complexity = 0.0; min_bucket = 1; maxdepth = 3; geom_yn = true; gt = .9; datafolderpath="../data/"; resultsfolderpath="../results/"
...
function run_single(;data::String=data,
	...
	if method =="localsearch" 
		...
		geom_search = geom_yn, geom_threshold = gt);
		run_time = @elapsed OptimalTrees.fit!(lnr, X, y);
	...
	score_optclust = optclust_score, score_kmeans_bestk=kmeans_scoredict[kmeans_best], 
	score_kmeans_kc=kmeans_scoredict[leaf_cnt], score_true=true_score,
	ari_optclust_kmeans=ari_optclust_kmeans, ari_true_kmeans=ari_true_kmeans, ari_true_optclust=ari_true_optclust,
	runtime = run_time)

	# filepath_lnr = joinpath(resultsfolderpath, "lnr-$dataset_name-$cr-$method-lnr.jld")
 #    @save filepath_lnr lnr

    OptimalTrees.writedot("$(resultsfolderpath)/$name_short-$cr-$method-geom$geom_yn$gt.dot", lnr);
	run(`dot -Tpng -o $(resultsfolderpath)/$name_short-$cr-$method-geom$geom_yn$gt.png $(resultsfolderpath)/$name_short-$cr-$method-geom$geom_yn$gt.dot`);

    filepath_accuracy = joinpath(resultsfolderpath, "results-$name_short-$cr-$method-geom$geom_yn$gt.csv")
